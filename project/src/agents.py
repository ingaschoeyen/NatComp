...
class Candidate():
    ...
    votes: int = 0  # Number of votes received by this candidate
    vote_history: list[int] = []  # History of votes received by this candidate

    def __init__(self, coords : Point, id : int = 0, approach: Approach = Approach.RANDOM, params: dict = default_cand_params):
        self.approach_weight = params.get("approach_weight", default_cand_params["approach_weight"])
        self.coords = coords
        ...
        self.approach_weight = default_cand_params.get('approach_weight', 0.1) if params is None else params.get('approach_weight', 0.1)  # Weight of the approach in the candidate's position update
...
class Voter():
    ...
    def __init__(self, coords: Point, strat : Strategy, params: dict, n_candidates : int = 0, id: int = 0):
        ...
        self.poll_weight = params.get("poll_weight", default_voter_params["poll_weight"])
        self.social_weight = params.get("social_weight", default_voter_params["social_weight"])
        self.coords = coords
        self.strat = strat
    ...
    def get_votes(self, candidates : list[Candidate], system : System, polls : Optional[list[float]] = None, dist_metric = distance_euclid):
        ...
        # If no previous polls exist, ask for honest (or random) opinion
            ...
            voter_strategy = Strategy.HONEST
